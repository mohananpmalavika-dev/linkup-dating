# TERMS OF SERVICE - DatingHub App

**Last Updated:** April 28, 2026  
**Effective Date:** May 1, 2026  
**Jurisdiction:** India (Indian Contract Act, Consumer Protection Act 2019), EU (GDPR), US (where applicable)

---

## 1. ACCEPTANCE OF TERMS

By downloading, installing, or using the DatingHub mobile application and website (collectively, "Platform"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the Platform.

**DatingHub is a dating and social networking platform for adults 18 years and older.**

---

## 2. USER ELIGIBILITY & REGISTRATION

### 2.1 Age Requirement
- You must be **at least 18 years old** to use DatingHub
- You represent and warrant that you are of legal age
- We use automated age verification and may request government-issued ID
- False age claims result in immediate account termination

### 2.2 Account Registration
- You are responsible for all activity under your account
- You must provide accurate, complete, and current information
- You must not register multiple accounts
- You are responsible for maintaining password confidentiality
- You agree to notify us immediately of unauthorized access

### 2.3 Prohibited Users
You cannot use DatingHub if you:
- Are a registered sex offender
- Have been convicted of any crime involving violence, harassment, or exploitation
- Have been permanently banned from any dating platform
- Are in a relationship and not authorized by your partner
- Are impersonating another person
- Are a minor

---

## 3. USER CONDUCT & COMMUNITY GUIDELINES

### 3.1 Prohibited Behavior
You agree NOT to:

**Safety & Harassment**
- Harass, threaten, intimidate, or abuse other users
- Engage in stalking, cyberbullying, or hate speech
- Discriminate based on protected characteristics
- Send unsolicited explicit content
- Engage in sexual harassment or coercion
- Attempt to blackmail or extort other users

**Fraud & Deception**
- Impersonate another person or entity
- Use catfish profiles or deceptive photos
- Use AI-generated or edited photos to deceive
- Mislead about relationship status, location, or age
- Use the platform for scams, fraud, or money laundering

**Illegal Activity**
- Attempt to sell drugs, weapons, or illegal services
- Facilitate human trafficking or exploitation
- Distribute malware or harmful code
- Use the platform for illegal gambling or betting
- Solicit nude photos from minors (illegal)
- Engage in any form of child exploitation

**Platform Abuse**
- Spam, bulk messaging, or automated scraping
- Use bots or unauthorized automation tools
- Sell or share account access
- Attempt to hack or reverse-engineer the Platform
- Circumvent security measures or rate limits
- Use VPN/proxy to hide real location (without explicit permission)

### 3.2 Content Standards
Your profile and messages must not contain:
- Explicit sexual content or pornography
- Hate speech or slurs
- Violence or graphic imagery
- Misinformation or conspiracy theories
- Religious extremism or radicalization content
- Links to malware or phishing sites
- Commercial spam or MLM content

---

## 4. CONTENT MODERATION & ENFORCEMENT

### 4.1 Our Moderation Process
- We employ automated systems and human reviewers
- Flagged content is reviewed within 24-48 hours
- Users can report abuse in-app via "Report" button
- Serious violations result in immediate account suspension

### 4.2 Enforcement Actions
**Warning:** First minor violation (spam, low-quality photos)  
**Temporary Suspension:** 24-72 hours for moderate violations  
**Permanent Ban:** Repeat violations or serious misconduct (fraud, harassment, illegal content)

### 4.3 Appeal Process
- You have 30 days to appeal suspension/ban
- Send appeal to: appeals@datinghub.app
- Include your account details and explanation
- Appeals are reviewed within 5 business days
- Final decision is binding and non-negotiable

---

## 5. INTELLECTUAL PROPERTY & USER CONTENT

### 5.1 Your Rights
- You retain all rights to content you upload
- You grant DatingHub a non-exclusive, royalty-free license to use your content for Platform operations, marketing, and analytics
- You can delete your account and associated content anytime

### 5.2 DatingHub's Intellectual Property
- All Platform code, design, logos, and trademarks are owned by DatingHub
- You may not reproduce, distribute, or modify Platform content without permission
- Unauthorized scraping or data collection is prohibited

### 5.3 Photo & Video Usage
- By uploading photos/videos, you confirm you have rights to them
- You represent that photos are recent, authentic, and of you (or with consent)
- DatingHub may use anonymized photos for platform improvement and advertising
- You can opt-out of promotional use in Settings

---

## 6. PRIVACY & DATA PROTECTION

### 6.1 Data Collection
We collect:
- Account info (email, phone, age, location)
- Profile data (photos, interests, preferences)
- Behavioral data (swipes, messages, matches)
- Device info (OS, device type, app version)
- Usage analytics and crash reports

### 6.2 Data Storage & Security
- Data encrypted in transit (HTTPS/TLS)
- Passwords hashed with bcrypt
- PII stored in encrypted fields
- Daily automated backups
- Access restricted to authorized staff
- No data sold to third parties

### 6.3 Third-Party Sharing
We share data only with:
- Payment processors (for transactions)
- Hosting providers (for infrastructure)
- Analytics services (anonymized data)
- Law enforcement (when legally required)
- Fraud detection services

### 6.4 User Rights (GDPR/DPDPA)
You have the right to:
- **Access:** Download your data in JSON format (Settings → Data Export)
- **Correction:** Update or correct your information
- **Deletion:** Request permanent account deletion (30-day cooling off period)
- **Portability:** Transfer your data to another service
- **Objection:** Opt-out of marketing communications
- **Complaint:** File a complaint with data protection authorities

---

## 7. PREMIUM SUBSCRIPTIONS & PAYMENTS

### 7.1 Subscription Plans
- **Free Tier:** 10 daily swipes, 1 message per match
- **Premium:** ₹99/month, ₹499/quarter, ₹999/year
  - Unlimited swipes
  - Message before matching
  - See who liked you
  - Advanced filters
  - Ad-free experience

### 7.2 Billing
- Charged via Razorpay to payment method on file
- Recurring charges on subscription renewal date
- Automatic renewal unless cancelled
- Refund within 48 hours of charge

### 7.3 Cancellation
- Cancel anytime in Settings → Subscription
- No refund for partial months/quarters/years
- Access ends immediately upon cancellation
- Data retained per privacy policy

### 7.4 Price Changes
- We may change prices with 30 days' notice
- Changes apply to new subscriptions, not active ones
- Existing subscriptions continue at current price until renewal

---

## 8. SAFETY & LIABILITY

### 8.1 First Date Safety
- DatingHub is NOT responsible for real-world interactions
- Use the Safety Kit: share location, set check-ins, SOS button
- Report unsafe behavior to police immediately
- DatingHub cannot verify user claims or guarantee compatibility

### 8.2 NO WARRANTIES
THE PLATFORM IS PROVIDED "AS IS" WITHOUT WARRANTIES, EXPRESS OR IMPLIED. WE DISCLAIM:
- Merchantability or fitness for a particular purpose
- Accuracy, completeness, or timeliness of information
- Uninterrupted or error-free service
- Results or outcomes from dating/matches

### 8.3 LIMITATION OF LIABILITY
TO THE FULLEST EXTENT PERMITTED BY LAW:
- DatingHub is not liable for any indirect, incidental, or consequential damages
- Our total liability is limited to the amount you paid in the past 12 months
- Some jurisdictions don't allow liability limitations

### 8.4 No Responsibility for User Conduct
DatingHub is not responsible for:
- Real-world interactions between users
- User injuries, trauma, or emotional distress
- Fraud, scams, or financial loss from other users
- Off-platform communication or meetings
- Third-party platform issues (payment processor, SMS service)

---

## 9. DISPUTE RESOLUTION

### 9.1 Governing Law
- These Terms are governed by laws of India
- Non-exclusive jurisdiction: Courts of Kochi, Kerala
- For EU users: GDPR applies regardless of location

### 9.2 Arbitration (India Users)
- Disputes resolved through arbitration (Arbitration and Conciliation Act, 1996)
- Single arbitrator unless claim exceeds ₹50 lakhs
- Seat of arbitration: Kochi, Kerala
- Language: English
- Arbitration fees borne equally

### 9.3 Escalation Process
**Before legal action:**
1. Send notice of dispute to: legal@datinghub.app
2. Good faith negotiation for 30 days
3. If unresolved, escalate to arbitration
4. Legal action only after arbitration exhausted

### 9.4 Class Action Waiver
- Disputes resolved individually, not as class actions
- You waive right to participate in class/collective actions
- Exception: Where prohibited by law

---

## 10. MODIFICATIONS TO TERMS

- DatingHub may update these Terms at any time
- Material changes notified 30 days in advance
- Continued use = acceptance of updated Terms
- If you disagree, stop using the Platform

---

## 11. CONSUMER PROTECTION (India)

**As per Consumer Protection Act, 2019:**
- Service provider: DatingHub (Kerala-based)
- Grievance Redressal Officer: grievance@datinghub.app (Response within 1 month)
- Escalation: District Consumer Commission (if unresolved)
- Consumer has 2 years to file complaint from date of service

---

## 12. CONTACT US

**For Support:**
- Email: support@datinghub.app
- In-app: Help → Contact Support
- Response time: Within 24 hours

**For Legal Notices:**
- Email: legal@datinghub.app
- Address: DatingHub, Kochi, Kerala, India

**For Appeals/Grievances:**
- Email: appeals@datinghub.app
- Response time: 5 business days

**For Privacy/Data Requests:**
- Email: privacy@datinghub.app
- See Privacy Policy for details

---

**© 2026 DatingHub. All rights reserved.**
These Terms are governed by the laws of **India**, specifically applicable laws in the user's state of residence.

### 9.2 Jurisdiction
- **Non-India Users:** Disputes resolved via binding arbitration in English language
- **India Users:** Disputes subject to courts in Kerala/user's state (if resident in India)

### 9.3 Dispute Process
1. Contact us: support@datinghub.app with complaint details
2. We respond within 7 business days
3. If unresolved, escalate to grievance officer
4. If still unresolved, pursue legal remedies

### 9.4 Grievance Officer (India)
**Name:** [To be appointed]  
**Email:** grievance@datinghub.app  
**Response Time:** 24 hours acknowledgment, 48 hours resolution commitment

---

## 10. TERMINATION & ACCOUNT DELETION

### 10.1 Account Deletion
- You can delete your account anytime via Settings
- 30-day cooling-off period before permanent deletion
- Photos/messages sent to others are not deleted
- You can cancel before day 30 to reactivate

### 10.2 Platform Termination
DatingHub may terminate or suspend your account if:
- You violate these Terms
- Requested by law enforcement
- Legitimate safety concerns
- Non-payment of disputed charges
- Inactivity for 2+ years

---

## 11. MODIFICATIONS TO TERMS

We may update these Terms anytime. Material changes receive 30 days' notice via:
- In-app notification
- Email to registered address
- Platform announcement

Continued use after notice means acceptance. If you don't accept changes, delete your account.

---

## 12. CONTACT US

**Support Email:** support@datinghub.app  
**Grievance Officer:** grievance@datinghub.app  
**Mailing Address:** DatingHub App, Kerala, India  
**Business Hours:** Monday-Friday, 10 AM - 6 PM IST

---

## 13. SEVERABILITY

If any provision is found invalid, the remaining provisions remain in effect.

---

## 14. ENTIRE AGREEMENT

These Terms, Privacy Policy, and Community Guidelines constitute the entire agreement between you and DatingHub.

---

**By clicking "I Agree" during signup, you acknowledge you have read, understood, and agree to these Terms of Service.**

Last Revised: April 28, 2026
